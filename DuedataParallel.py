...
#print(patents)
patent_number = patents[0]
#print(patent_number)
patent_claim = patents[3]
patent_abstract = patents[2]
patent_title = patents[1]
del patents
            ...
            if type(company_id).__name__ == 'Series':
                ...
                for item in company_id['assignee_id']:
                    if str(assignee.ix[str(item)]['organization']) != 'nan':
                        inf_company += assignee.ix[str(item)]['organization'] + ';'
                    else:
                        inf_company += assignee.ix[str(item)]['name_last'] + ' ' + assignee.ix[str(item)]['name_first'] + ';'
        except KeyError as e:
            pass
        except Exception as e:
...
if __name__ == '__main__':
    ...
    for i in range(process):
        if i != process - 1:
            ...
            result.append(p.apply_async(InCo, args=(inf,)))
        else:
    ...
    for i in tqdm(range(patent_length)):
        if inf_inventor[i] != '' and inf_company[i] != '':
            patent_inf.append([patent_number[i], patent_title[i], patent_abstract[i], patent_claim[i], inf_inventor[i],
                               inf_company[i]])

    name = ['number', 'title', 'abstract', 'claim', 'inventor', 'company']
